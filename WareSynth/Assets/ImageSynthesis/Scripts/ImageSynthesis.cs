...
public class ImageSynthesis : MonoBehaviour
{
    ...
    public void Save(string path, string filename, int width = -1, int height = -1)
    {
        // capturePasses[0].camera = GetComponent<Camera>();
        // for (int q = 1; q < capturePasses.Length; q++){
        //     capturePasses[q].camera = CreateHiddenCamera(capturePasses[q].name);
        ...
        // StartCoroutine(
        //     WaitForEndOfFrameAndSave(filename, filenameExtension, width, height));
        Save(path, filename, filenameExtension, width, height);
    }
